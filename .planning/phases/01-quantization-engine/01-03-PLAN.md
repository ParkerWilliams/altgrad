...
<task type="auto">
  ...
  <files>altgrad/__init__.py, altgrad/quantization/__init__.py</files>
  <action>
    First, ensure directory structure exists:
    - mkdir -p altgrad/quantization (create altgrad/ and altgrad/quantization/ directories if they don't exist)
    - mkdir -p tests (create tests/ directory if it doesn't exist)
