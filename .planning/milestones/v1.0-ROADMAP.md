# Milestone v1.0: Research Infrastructure
...
**Key Decisions:**
- E3M4 bias=1 for range ~0.06 to ~124
- Round-to-nearest-even for FP8 tie-breaking (IEEE 754)
- E0M7 as pure fixed-point with sign-magnitude representation
- STE passes gradients unchanged through quantization
- Simulated FP8 quantization (not native FP8 tensor cores)
- Skip patterns approach for weight tying preservation
- Stiffness preconditioning BEFORE momentum in optimizer
- Stall ratio = 1 - (flips / updates) for gradient effectiveness

**Issues Resolved:**
- Format registry correctly handles all 5 FP8 formats
- Gradient flow through quantization verified with STE
- Weight tying preserved during model surgery
- Classifier-specific rank monitoring with stricter thresholds

**Technical Debt:**
- Reports in placeholder mode until experiments run on H100
- One edge case test with equality assertion (1.0 > 1.0)
